...
export default function Events() {
    ...
    return <div className="page"><Navbar /><main className="page-content">
        ...
        <div className="card" style={{ padding: 16, overflowX: 'auto' }}>{loading ? <div style={{ padding: 30, textAlign: 'center' }}>Đang tải...</div> : <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 650 }}><thead><tr style={{ textAlign: 'left', borderBottom: '1px solid var(--color-border)' }}><th style={{ padding: 12 }}>Ngày</th><th style={{ padding: 12 }}>Sự kiện</th><th style={{ padding: 12 }}>Thành viên</th><th style={{ padding: 12 }}>Mô tả</th><th style={{ padding: 12 }}>Thao tác</th></tr></thead><tbody>{events.map(item => <tr key={item.id} style={{ borderBottom: '1px solid var(--color-border)' }}><td style={{ padding: 12 }}>{new Date(item.eventDate).toLocaleDateString('vi-VN')}</td><td style={{ padding: 12 }}><strong>{item.title}</strong><div style={{ fontSize: '0.78rem', color: 'var(--color-text-muted)' }}>{typeName[item.eventType] || item.eventType}{item.isRecurringYearly ? ' · Hàng năm' : ''}</div></td><td style={{ padding: 12 }}>{item.memberName || 'Toàn dòng họ'}</td><td style={{ padding: 12 }}>{item.description || 'Không có'}</td><td style={{ padding: 12 }}>{canManage && <button className="btn btn-sm" style={{ color: '#ef4444' }} onClick={() => remove(item.id)}>Xóa</button>}</td></tr>)}</tbody></table>}{!loading && events.length === 0 && <div style={{ padding: 30, textAlign: 'center', color: 'var(--color-text-muted)' }}>Chưa có sự kiện.</div>}</div>
    </main></div>;
}
