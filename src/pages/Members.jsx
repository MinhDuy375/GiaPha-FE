                    ...
                    {loading ? <div style={{ padding: 40, textAlign: 'center', color: 'var(--color-text-muted)' }}>Đang tải danh sách...</div> : (
                        <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 760 }}>
                            ...
                            <tbody>{filteredMembers.map(member => <tr key={member.id} style={{ borderBottom: '1px solid var(--color-border)' }}>
                                ...
                                <td style={{ padding: '13px 16px' }}><div>{member.birthYear || '?'} - {member.deathYear || (member.isAlive ? 'nay' : '?')}</div><small style={{ color: 'var(--color-text-muted)' }}>Âm: {formatMemberLunarDate(member.birthDateLunar, member.birthLunarDay, member.birthLunarMonth, member.birthLunarYear)}{!member.isAlive && ` - ${formatMemberLunarDate(member.deathDateLunar, member.deathLunarDay, member.deathLunarMonth, member.deathLunarYear)}`}</small></td>
